...
public class ClientRunnable implements Runnable {
	 ...
	 */
	private boolean checkMessages(boolean terminate) throws JsonProcessingException {
	...
	private void handleDisplayMsg(Message msg) {
		...
		if (messageChecks(msg)) {
			...
			if ((msg.isBroadcastMessage()) && (!broadcastMessageIsSpecial(msg))) {
				// Check for our "special messages"
				if ((msg.getText() != null) && (msg.getText().compareToIgnoreCase(ServerConstants.BOMB_TEXT) == 0)) {
					initialized = false;
	 ...
	 * @return
	 */
	private Message createGroupSubpoena(Message msg, LocalDate fromDate, LocalDate toDate, Boolean valid) {
		Message ackMsg;
		Subpoena sb;
	 ...
	 * @return
	 */
	private Message createUserSubpoena(Message msg, LocalDate fromDate, LocalDate toDate, Boolean valid) {
		Message ackMsg;
		Subpoena sb;
	...
	private void searchMsg(Message msg) {
		List<String> messages = userService.getMessages(msg.getText(), msg.getMsgRecipient(), msg.getName());
	...
	private void sendGroupMsg(Message msg) {
		...
		if (group == null || !group.getListOfUsers().contains(msg.getName())) {
			...
			String mg = timestamp + " " + getIP() + " [" + user.getUsername() + "@" + msg.getMsgRecipient() + "] "
					+ msg.getText();
			this.enqueueMessage(Message.makeUID("" + timestamp));
	...
	private void sendPrivateMsg(Message msg) {
		if (userService.findUserByUsername(msg.getMsgRecipient()) != null) {
			...
			String m = timestamp + " " + getIP() + " PRIVATE " + msg.getMsgRecipient() + " " + msg.getText() + OFFLINE;
